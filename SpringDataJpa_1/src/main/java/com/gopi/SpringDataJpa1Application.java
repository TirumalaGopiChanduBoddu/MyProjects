...
public class SpringDataJpa1Application implements CommandLineRunner{
	...
	public void run(String... args) throws Exception {
		...
		//createProducts();
		//updateProduct();
		//findByStockKeepingUnits();
		//findByStockKeepingUnitsAndName();
		//findByStockKeepingUnitsOrName__IndexParam();
		//findByStockKeepingUnitsOrName__NamedParam();
		//findByStockKeepingUnitsOrNameSQL_IndexParam();
		//findByStockKeepingUnitsOrNameSQLNamedParam();
		//findByStockKeepingUnits_NamedQuery();
		findByName_NamedNativeQuery();
	}

	private void createSingleProduct() {
		Product prod =createProduct("SKU-001", "Apple iPhone 14", "Latest model of iPhone with A15 Bionic chip, 6.1-inch display, and improved camera system.", 
				new BigDecimal("999.99"), "http://example.com/iphone14.jpg");
		productRepository.save(prod);
	}
	
	
	public void updateProduct() {
		Product prod = productRepository.findById(16L).get();
		prod.setActive(false);
		productRepository.save(prod);
		
	}
	
	private void findByStockKeepingUnits() {
		Product prod = productRepository.findBySku("SKU-014");
		System.out.println(prod);
	}
	
	public void findByStockKeepingUnitsAndName(){
		Product prod = productRepository.findBySkuAndName("SKU-014" , "Dyson V11");
		System.out.println(prod);
	}
	
	
	public void findByStockKeepingUnitsOrName__IndexParam() {
		Product prod = productRepository.findByStockUnitsorName_IndexParam("SKU-018", "Anker PowerCore 10000");
		System.out.println(prod);
	}
	
	public void findByStockKeepingUnitsOrName__NamedParam(){
		Product prod = productRepository.findByStockUnitsorName_NamedParam("SKU-010", "GoPro HERO10");
		System.out.println(prod);
	}
	
	
	public void findByStockKeepingUnitsOrNameSQL_IndexParam() {
		Product prod = productRepository.findByStockUnitsorNameSQL_IndexParam("SKU-020", "Canon EOS R5");
		System.out.println(prod);
	}
	
	public void findByStockKeepingUnitsOrNameSQLNamedParam() {
		Product prod = productRepository.findByStockUnitsorNameSQL_NamedParam("SKU-012" ,"Kindle Paperwhite");
		System.out.println(prod);
	}
	
	public void findByStockKeepingUnits_NamedQuery() {
		Product prod = productRepository.findBySKU("SKU-001");
		System.out.println(prod);
	}
	
	public void findByName_NamedNativeQuery() {
		Product prod = productRepository.findByName("GoPro HERO10");
		System.out.println(prod);
	}
